...
- [x] **Kurv** `kurv_screen.dart` at `/bergen/kurv`, tab 2 of the shell: **Seilas · kassen** header (≈L4976: "Bryggen → Kassen", "Ægil ror til bryggen"); Levering / Henting toggle; lines with quantity; empty state ("Kurven er tom — skal vi finne noe i Bergen?", "Bla gjennom Bergen", "Tilbud i kveld"); "Legg til noe mer – Glemte du drikke?" from the store's drinks category; Endre rows (address / time / payment "Betales ved bestilling"); "Beskjed til budet" + "Flere valg"; pickup note; tips 0/15/25/40 ("Alt går uavkortet til budet"); SAMMENDRAG incl. "Lagt i kurven av Ægil … 312 kr · Angre" (reads the cart's `added_by = aegil` lines — agil-3 writes them through the existing cart API; agil-1 just renders + `BergenUndoPill`); "DØREN · FOR BUDET" note with the "Tolk" chip (**C4, agil-3 — the chip pushes `kAegilRoute` with `intent=door`; no local logic**) and "Kode ved levering" toggle → `ops_delivery_codes` via the existing checkout; gift recipient "Overrask / Si fra"; "Å BETALE NÅ" total with "Gir N kr tilbake i Ærend-kroner"; footer "Ærendet krysser Vågen med {bud}" only once a courier is assigned; pay button = the existing Vipps flow (`endPointVippsInitiate`).
- [x] Sheets via `BergenArk`: **Adresse** ("Hvor skal ærendet?" + door note), **Ny adresse** (uses the existing address drawer `ae_address_drawer.dart` inside), **Levering** ("Når vil du ha det?" Så fort som mulig / 18:30 / 19:30 — slots from `ops_store_hours`), **Betaling** (Vipps / Visa / Apple Pay / Google Pay from the existing card list).
- [x] **Coverage at address change** (geo spec §4): call `GET /api/geo/coverage?lat=&lng=` when flag `geo.customer.coverage` is on; **404 or flag off → skip silently** (agil-3's endpoint may not exist on this branch yet). Not covered → "Vi leverer ikke hit ennå — si fra, så gir vi beskjed" with one-tap `POST /api/geo/waitlist`.
- [x] **Self-delivery checkout rule** (spec §8.2): a 422 `PD_OUTSIDE_RADIUS` from checkout shows "Butikken leverer ikke hit" and offers pickup. The customer's fee line is unchanged whoever delivers (spec §5).
- [x] **Purchase sequence** `kjop_sekvens.dart` (`kjopSteg2–4` ≈L5249): steps 2–3 are placeholders in the design — implement as the Vipps return → "Bekreftet" transition; step 4 **Levert · vervebillett** ("ÆREND-BILLETT {kode} – Gi 100 kr, få 100 kr", "Del billetten" / "Kopier", code from `GET /api/points/me/referral`, guarded) shown once per order after Levert.
- [x] **Bestillingsdetaljer** `bestilling_sheet.dart` at `/bergen/bestilling/{id}` (≈L5641): Sammendrag / Detaljer tabs, Vipps timestamp, ORDRENUMMER + ÆREND-ID, store address, "Kvittering", "Meg · Bestillinger" (→ `/bergen/meg/bestillinger`, agil-3), "Kontakt kundeservice" (→ `/bergen/kundeservice`). Data: `ops.customer.orders`.
- [x] Copy `kasse_copy.dart` (`KasseCopy`, `a1_kasse_*`); tests `test/bergen/kasse_test.dart`: empty / filled / pickup / gift; tip math; the Ægil-added line and Angre; coverage skipped on 404; the 422 `PD_OUTSIDE_RADIUS` → pickup offer; the code request after placement; Bestillingsdetaljer; the referral ticket once per order and hidden without the route. Data: `lib/data/ops/kasse_models.dart`, `lib/networking/ops/ops_kasse_api.dart` (the existing cart / preview / address / place-order / Vipps-initiate calls). Backend: `OrderIntakeService` + `LegacyOrderObserver` (one `chore(shared)` append in `AppServiceProvider`): a legacy booking gets `ops_state = placed`, `ops_origin`, `ops_policy_version`, `ops_code` and an `order.placed` event at creation — `OrderIntakeTest`; `POST …/orders/{id}/code` → `ops.customer.code` (`DeliveryProofService::assignProof` with `customer_requested_code`), both registered in the contract §3.2 and the registry. The Ægil-added lines are the cart rows carrying a Snurre conversation (`snurre_conversation_id`) — the cart API has no `added_by` field; agil-3's writes through the cart API will carry it. The flag `geo.customer.coverage` is not readable by the app (no flags endpoint) — a 404 on the coverage route gives the same silent skip.

**Design ledger**

| Screen | Design block | Differences |
|---|---|---|
| Kurv | ≈L4973 | Compared code-side. Same: Seilas · kassen header (Bryggen → Kassen with the boat, "Ægil ror til bryggen"), Levering / Henting thumb, lines with quantity, the empty state with Ægil and "Bla gjennom Bergen", "Legg til noe mer · Glemte du drikke?" (→ the store page), Endre rows (adresse / tid / betaling "Betales ved bestilling"), "Beskjed til budet" + Flere valg, pickup note and Hentetid chips, tips 0/15/25/40 with "Alt går uavkortet til budet", SAMMENDRAG with the Ægil line + Angre (`BergenUndoPill` restores), DØREN · FOR BUDET with the Tolk chip (→ `kAegilRoute` `intent=door`, no local logic), Kode ved levering, gift Overrask / Si fra, Å BETALE NÅ, "Alt inkl. mva", the pay button on Vipps. **Differences accepted:** (1) "Tilbud i kveld · Rett i kurven" specials on the empty state are not shown (no store context in an empty cart); (2) "Gir N kr tilbake i Ærend-kroner" hidden — no points rule on this tree (`GET /api/points/rules` is guarded); (3) "Ærendet krysser Vågen med {bud}" is a Sporing-time line — no courier at checkout; (4) card payment opens the existing Stripe checkout (the design's Visa / Apple Pay / Google Pay rows are not app payment methods; Apple/Google Pay do not exist in the app); (5) "Se fullt kart" for pickup is a toast until a map sheet exists (Phase 6's `kArkErKart`). |
| four sheets | ≈L6913–7142 | Adresse: "Hvor skal ærendet?", "Butikker og priser følger adressen.", saved addresses as rows, "Legg til en adresse · Hytta, kjæresten, foreldrene …", the door kicker. Ny adresse: the existing drawer. Levering: "Når vil du ha det?", Levering / Henting chips, "Så fort som mulig · Innen HH:MM · a–b min", two :30 slots with the design's lines, "Bruk dette". Betaling: "Hvordan vil du betale?", the fee line, Vipps · {tlf}, Kort. **Differences:** slots are computed from now + ETA rather than `ops_store_hours` (the hours are not on the preview response; the store's close time is on the store page). |
| Levert · vervebillett | ≈L5249 | ÆREND-BILLETT {kode} on the Ark's copy block with Kopier, VERV EN VENN, "Gi N kr, få N kr" and the line (N from `points_for_me`), "Del billetten" (share sheet) / Lukk; once per order (`a1_kasse_billett_{id}` pref); hidden when the referral route is not here. Steps 2–3 = `KjopBekreftetScreen` after the Vipps return (check burst, "Bekreftet", "Følg ærendet" → `/bergen/sporing/{id}`). |
| Bestillingsdetaljer | ≈L5641 | Sammendrag: mode, address, BESTILLING line, N varer · total, BETALT MED VIPPS · total · time, Kvittering (copies the order number) / Klar. Detaljer: ORDRESTATUS, Din bestilling lines, Totalsum, Betaling (Vipps · date time · Betalt), Butikken + address, ORDRENUMMER + ÆREND-ID, TIDSSTEMPEL, Meg · Bestillinger (agil-3), Kontakt kundeservice (`/bergen/kundeservice`, Phase 6). **Differences:** payment is shown as Vipps for every order (the order summary carries no payment method; TODO(api)); no receipt PDF exists — Kvittering copies the order number. |

**Acceptance**
- [x] Lands in `ops_order_events` with `ops_origin = app` and `policy_version`: asserted by `OrderIntakeTest` through the same model observer the legacy checkout triggers (`UserProductBooking::save()` → `order.placed`, origin, policy version, code). **Not run in this session:** the Vipps test-environment round trip (no Vipps credentials here) — the app's Vipps return now lands on `KjopBekreftetScreen` → `/bergen/sporing/{id}`. Commit `Phase 5: Kurv og kasse`.
...
- [x] **Sporing** `sporing_screen.dart` at `/bergen/sporing/{id}` (≈L5287–5760, plus `spHentKlar` ≈L5500 and `spPaaVei` ≈L5457): data from `ops.customer.tracking` polled every 10 s, or the events channel when broadcasting is on (`private-customer.{id}`); top line "Live · {stadie}", ETA, "om N min"; **Sporing · stadier** stepper (`BergenStepper`, labels by `mode`); **stadieskifte** overlay ("Steg N av 4", "+X poeng" — points from `GET /api/points/me` delta, guarded) on every stage change; stage cards **Bekreftet** (receipt), **Tilberedes** (ticket, "På komfyren · 6 min igjen" from `ops_predicted_ready_at`), **På vei** (map with the route; live marker only when `live_position != null`; bike or car from `courier.vehicle`), **Klar for henting** (pickup variant: "Den står i disken. Si «{navn}»", "Vis veien" → map sheet `kArkErKart`), **Levert**; **Ægil-veileder** line ("Ægil · følger ærendet ditt" + `spNesteHint`); **Avslutt** ("Avslutt bestillingen" at stage 3 → `/bergen/levert/{id}`; before that "Fjordfiske mens du venter" → `/bergen/fjordfiske`); gift variant "{navn} venter på kaien".
- [x] **Completion layer** (≈L7294–7326): **Bud-identitet** pill (courier initial + "BankID-verifisert" when `courier.verified`, or the store icon when `delivery_actor == partner`); **Leveringskode** (7×7 visual code from `qr_payload` + PIN; offline → PIN only, "Uten nett vises bare PIN."); **Valg som venter** ("Torgboden har ikke sett ordren" when `unseen_by_store`; Vent → no-op + toast, Avbestill → `ops.customer.problem kind=cancel` → full refund copy); `BergenOfflineBanner` on connectivity loss with the last payload.
- [x] **Self-delivery variants** (spec §5, every line): label "Leveres av {butikknavn}" and no courier identity when `partner`; stages from the store-emitted statuses (server-derived — assert the app never maps states itself); ETA from the store estimate; **no live map** (route/destination only, no marker); contact routing "Ring butikken" / "Melding til butikken"; "Finner ikke døra" and "Noe mangler" route to the store; PIN shown as today; no courier notifications ("Butikken er på vei" variant — the notification copy keys `a1_sporing_notif_store_*`).
- [x] **"Finner bud"** (agents spec §7): when `finding_courier`, the top line reads the `order_status_finding_courier` copy ("Finner bud") and the Ægil-veileder hint says a courier is being found; never for partner orders.
- [x] **Hjelp sheet** `hjelp_sheet.dart` at `/bergen/sporing/{id}/hjelp` (`sheetHjelp` ≈L7000–7130): main (contact card "På vei · 4 min unna · sykler siden mars · 4,9" when courier, store card when partner; Ring / Send melding), **Ring {{ kontaktRolle }}** (in-app call UI: Demp / Avslutt / Høyttaler → `ops.customer.contact kind=call`, which today creates the relay record and, with no provider, shows the store/courier number masked per `ops_relay_calls`), **Melding til {{ kontaktRolle }}** (thread with typing indicator → `ops.customer.contact kind=message`), **Finner ikke døra** ("Dette er det {navn} ser nå", address, directions, Send / Ring / Skriv selv → `problem kind=door`), **Noe mangler** (tap the missing items → `problem kind=missing`; "Vipps-refusjon innen 2 min eller ny levering"), **Kundeservice** (≈L7114: hours, chat, 55 00 12 34, "Ordre #… er allerede lagt ved" → the existing live chat), **Bekreftet** (sent confirmation, "Ferdig"). `kontaktRolle` = "butikken" when partner else "bud".
- [x] **Kundeservice** standalone at `/bergen/kundeservice` (same body, no order attached) — reached from Bestillingsdetaljer and Meg (agil-3 pushes it).
- [x] **Levert** `levert_screen.dart` at `/bergen/levert/{id}` (≈L5881–5940): "18:11 Levert. Håper det smaker. to minutter før tiden" (from `promised_end` vs the delivered event), **Poeng for denne ordren** line (≈L5912: "+34 poeng", league gap, first-time bonus, "Du støttet en familiedrevet butikk…" — from `GET /api/points/me/ledger?order={id}`, guarded; hidden on 404), "Levert til deg · koden ble bekreftet av {bud|butikken}", "Takk til {bud}" (courier only), "Hvordan gikk det?" (existing rating), "Noe galt med bestillingen?" → Hjelp, "Ferdig".
- [x] Retire `lib/screens/tracking/*`: `DeliveryCodeCard` and `OpsTrackingStatus` become the Leveringskode card and the stepper's status line inside `sporing/`; delete the old files and their tests once the new ones cover the same assertions.
- [x] Push notifications (`push_notification_service.dart`, food-delivery order types) and the Hjem's boat ("PÅ VEI NÅ") open `/bergen/sporing/{id}` (the legacy TrackOrder stays the fallback when the route is absent). `deliveries_order_detail.dart` has no tracking button of its own — its only exit is the close orb back to the Hjem — so nothing to rewire there; the Bestillingsdetaljer screen (`/bergen/bestilling/{id}`) is reached from Sporing's Sammendrag / Detaljer chips.
- [x] Demo panel triggers (debug builds, registered when Sporing opens): new order → Bekreftet and next stage drive `POST /api/ops/orders/{id}/transition`; unseen shop leaves the order accepted for the local `ops:sweep` to escalate (the payload's `unseen_by_store` then shows Valg som venter); code OK and PIN wrong ×3 drive the new `POST /api/ops/proof/orders/{id}/pin` (`ops.proof.pin`, registered in the contract §3.2 and the registry, `ProofRouteTest`); offline/online toggles the screen's fetch and shows `BergenOfflineBanner` with the last payload. No fake state.
- [x] Copy `sporing_copy.dart` (`SporingCopy`, `a1_sporing_*`, the spec §7 key `order_status_finding_courier` verbatim, the `a1_sporing_notif_store_*` keys); tests `test/bergen/sporing_test.dart`: 32 stage cases (8 states × 2 actors × 2 modes) rendered from server payloads, the partner assertions (no courier identity, no live marker, "Leveres av", store contact), Finner bud, the identity pill, the delivery code card (the retired `DeliveryCodeCard` assertions ported: PIN spaced, reason, no leave-at-door, visual only when issued and online), Valg som venter → wait, the Hjelp states (melding → contact, dør / mangler → problem, kontaktRolle butikken for partner, Kundeservice standalone), Levert (early line, points only when the ledger answers, thanks for courier only, rating). Data: `lib/data/ops/tracking_models.dart`. Polling every 10 s; there is no socket client in the app, so the events channel is not wired (TODO: Soketi client).

**Design ledger**

| Screen | Design block | Differences |
|---|---|---|
| Sporing stages | ≈L5287–5760 | Compared code-side. Same: "Live · {stadie}" top line with the Hjelp pill, ETA label + big time (window / arrival / delivered time; "om N min"), the four-step stepper with labels by mode, the Ægil-veileder card with the stage line and "Neste: … " hint, the stage cards (Bekreftet: MOTTATT, store, code, address; Tilberedes: "Æ-42K · kl. 18:05", "På komfyren · N min igjen" from `predicted_ready_at`; På vei: the map with the route and the courier marker only with `live_position`, bike / car from `courier.vehicle`; Klar for henting: "Den står i disken. Si «{navn}»" + Vis veien → map sheet; Levert), Sammendrag / Detaljer chips, Avslutt at stage 3, "Fjordfiske mens du venter" before, the gift line, the stadieskifte overlay with "Steg N av 4" and "+X poeng" when `points/me` moved. **Differences accepted:** (1) the design's 3D-tilt parallax on the cards and the drawn Bergen skyline behind the map are not reproduced (the map is Google Maps); (2) the +8/+8/+8/+34 points per step are the design's — the overlay shows the real delta from `GET /api/points/me` (hidden when the route is not here); (3) "Ankommer om mm:ss" countdown is "om N min" from the window; (4) SPART TID is not shown (no timing source). |
| completion layer | ≈L7294 | Bud-identitet pill (initial or avatar, name, BankID-verifisert, vehicle; the store icon and "Leveres av {butikk}" for partner — never a courier name), Leveringskode (7×7 visual from `qr_payload` + PIN; PIN only offline with "Uten nett vises bare PIN."; reason line; "Posen kan ikke settes igjen ved døren"; locked / confirmed states), Valg som venter ("{butikk} har ikke sett ordren", Vent → `problem kind=wait`, Avbestill → `kind=cancel` with the full-refund copy), the offline banner with the last payload. **Difference:** the design's "Mens du var borte" card in this layer is agil-3's (`mensDuVarBorteCard` on the Hjem). |
| Hjelp sheet states | ≈L7000–7130 | main (contact card — courier or store by `delivery_actor` — Ring / Send melding, VANLIGE SPØRSMÅL rows), Ring {{ kontaktRolle }} (in-app call UI with Demp / Avslutt / Høyttaler over `contact kind=call`; with no relay provider the masked proxy number is shown and dialled), Melding til {{ kontaktRolle }} (thread, quick replies, typing indicator, `kind=message`), Finner ikke døra (address, directions field, Send / Ring / Skriv selv → `problem kind=door`), Noe mangler (tap the items → `kind=missing`, the Vipps refund line), Kundeservice (hours, Chat med oss → the existing contact screen, Ring 55 00 12 34, "Ordre #… er allerede lagt ved"), Bekreftet (Ferdig). **Differences:** the courier card's "4 min unna · sykler siden mars · 4,9" needs fields the payload does not carry (tenure, rating) — the card says "På vei til deg"; the live chat is the existing Kontakt oss screen (the support chat is hidden in the app today). |
| Levert | ≈L5881 | "18:11 Levert. Håper det smaker." with "· N minutter før tiden" from `promised_end` vs `delivered_at`, Poeng for denne ordren (+N, league gap, first-time bonus, the story line — from `GET /api/points/me/ledger?order=`, hidden on 404), "Levert til deg · koden ble bekreftet av {bud|butikken}" (or "… av" without a code), "Takk til {bud}" (courier only), "Hvordan gikk det?" stars on the existing rating call, "Noe galt med bestillingen?" → Hjelp, Ferdig; the referral ticket once per order. **Difference:** `delivered_at` is read from the payload's `delivered_at` when present, else the screen's open time (TODO(api): expose the delivered event's `occurred_at` on the payload). |

**Acceptance**
- [x] Every state × actor × mode is rendered from server-shaped payloads in `sporing_test.dart` (the app never derives the stage); the guarded default path is asserted on the backend (`CustomerTrackingTest`) and the partner variant in the widget tests from the contract's payload. **Not run in this session:** the simulator walk-through against `php artisan serve` (the demo panel's triggers are wired for it). `flutter test` green (405); commit `Phase 6: Sporing, Hjelp og Levert`.
...
- [x] Contract §6, executed 2026-09-25 (no conflict outside §2.4):
  1. On `agil-3` — Hare-AdminPanel: `git checkout agil-3 && bash scripts/rebuild_test_db.sh && php artisan test` → the same 6 pre-existing unit failures as `agil-1`, nothing else; `php artisan test --filter=ContractNamesTest` → 153 passed, 88 skipped (agil-1's entries, as expected there). Aerend-app: `flutter test` on `agil-3` → 373 passed. `git diff --name-only <merge-base> origin/agil-3`: everything in agil-3's §2 columns plus §2.4 appends (`config/app.php` +2/−0, `.env.example` +23/−0, `app/Console/Kernel.php` +13/−0, `dugnad-i18n.js` +76/−0, `routes/api.php`, `AppServiceProvider`), **and two additive edits in agil-1's columns**: `resources/views/admin/pages/super_admin/ops/_tabs.blade.php` (+5: the Egenlevering tab, guarded by `Route::has`) and `tests/Feature/Ops/DispatchTest.php` (the guarded-default assertion becomes a skip once `pd_delivery_actor` exists — what §6 step 5 needs). Accepted as additive; recorded here.
  2. On `agil-1`: `php artisan test` → the 6 unit failures + 6 pre-existing `DemoScenarioTest`/`EdgeCaseTest` (pickup-key PEMs); `flutter test` → 426 passed; `ContractNamesTest` green with agil-1 entries asserted.
  3. `git merge-tree --write-tree agil-1 origin/agil-3`: backend clean; app had one conflict, `lib/screens/bergen/poeng/poeng_entry.dart`, caused by a `dart format` pass on agil-1 over agil-3's seam file in Phase 2 — fixed on agil-1 by restoring the Sync C text (`9ed6e37`), after which the preview was clean.
  4. `git merge --no-ff origin/agil-3` on `agil-1`: Hare-AdminPanel `75f14b2`, Aerend-app `5fffe95`. `scripts/rebuild_test_db.sh` applied agil-3's nine `a3_` migrations. `ContractNamesTest` on the merged tree: 5 skips left, all agil-3's reserved entries (four `agent` names and the `delivery_income` payout-line kind — agil-3's "asks of agil-1"). Granted on agil-1: the `agent` kind greps every seeder; `PayoutLine::KINDS` gains `delivery_income`; the `dart` kind asserts the route in the app's map file instead of skipping; entries flipped to built. Then `php artisan ops:contract-check --strict` → **"Every registered name is real. Merge-ready."** (the command now runs through `artisan test` in a scrubbed child environment — spawned from artisan it inherited `DB_DATABASE=db_hare`). Full suites: backend the same 6 unit failures only; app 447 passed (see l10n below).
  5. `EventContractTest|MergeIntegrationTest|DispatchTest|DispatchSkipTest` → 51 passed, 1 skipped (`DispatchTest`'s guarded-default case, by agil-3's arrangement; `DispatchSkipTest` covers the present-column branch).
  6. Nothing flipped: `ops:flags list` shows every `ops.customer.*`, `agt.*`, `geo.*`, `pd.*` key present and not seeded (off).
- [x] **l10n consolidation** — 667 keys moved into `intl_no.arb` / `intl_en.arb` (`ops_utforsk_*`, `ops_sok_*`, `ops_butikk_*`, `ops_kasse_*`, `ops_sporing_*`, `pts_*`, `aegil_*`, `meg_*`, and `order_status_finding_courier` verbatim; `n == 1 ? … : …` became ICU plurals; interpolations became typed placeholders), `flutter gen-l10n` run (through `l10n.yaml`, output in `lib/l10n/`), and every copy class now reads `AppLocalizations` through the app's `languages` handle (`SokCopy.a1_sok_title => languages.ops_sok_title`). **Two deliberate departures from the bullet:** (1) the copy *files stay* as thin facades — the 700+ call sites keep their `XCopy.key` names, which is what the contract's `a1_*` / `a3_*` traceability rule is for, and the facade is the single place the ARB key is named; (2) agil-3's 195 `static const String` members are *mirrored*, not replaced — their screens use them in `const` widget trees (34 `const` sites across `aegil_screen`, `minne_screen`, `bestillinger_screen`, `konto_screen`…), so the ARB holds the entry and the Dart constant stays as the compile-time mirror. Their non-const members (functions) are facades like agil-1's. The 11 members kept in Dart are the `_t`/`kr` helpers and four functions with logic (`a1_kasse_bet_vipps`, `a1_butikk_km`, `a1_sporing_levert_min_for`, `a1_sporing_meld_mangler`). agil-3's test harness (`test/a3/a3_fakes.dart`) now loads `languages` (NO); `flutter test` green (447). Tool: the consolidation script is reproducible (`scratchpad/l10n_consolidate.py` in the session; not committed — it is a one-off).
- [x] **Design review ledger** — every screen has a row: agil-1's in this file's phase ledgers (Phase 2: Utforsk, Nytt fra butikkene, Hjem entry points; Phase 3: Søk empty / treff / wish banner; Phase 4: Kategori (+Gaver, Mote), restaurant page, fashion / gift page, Klede sheet, food product sheet, Poseautomaten; Phase 5: Kurv, the four sheets, Levert · vervebillett, Bestillingsdetaljer; Phase 6: Sporing stages, completion layer, Hjelp states, Levert), agil-3's Phase 7 rows in `plans/AGIL-3-PLAN.md` (Meg, Favoritter, Konto, Bestillinger, Varsler, Mens du var borte, Premiehylla, Premie, Ægil velger, Fløyen-ligaen, Nivåopprykk, Fjordfiske, Napp-kort, Ægil, Det Ægil vet, Brett). **Caveat, stated plainly:** agil-1's rows are code-side comparisons against the design blocks (structure, copy, states, data sources); no device or simulator screenshot was taken in this session, so pixel-level items (spacing, the 3D tilts, the drawn scenes) are listed as accepted differences rather than verified.
- [x] Flags: `ops:flags list` on the merged tree shows every new key (`ops.customer.*`, `agt.*`, `geo.*`, `pd.*`) present and not seeded — off; `docs/OPS_ROLLBACK.md` §5 gained "The customer's Bergen screens": the five customer flags after the storefront stage, `ops.customer.sporing` last.
- [x] `plans/AGIL-1-REMAINING.md` updated with a post-merge header (§1 pointer: what is done, what stays human, the asks granted and the two agil-3 asks that are follow-ups).

**Acceptance**
- [x] Both suites green on the merged tree (backend: only the 6 pre-existing unit failures; app: 447); `ContractNamesTest` no skips; `ops:contract-check --strict` exit 0; commit `Phase 8: merge, l10n, review`.
