...
import 'package:aerend_customer/screens/campaign/campaign_list_screen.dart';
...
class HomeV1State extends State<HomeV1>
                      ...
                      children: [
                        ...
                        StreamBuilder<ApiResponse<HomeCatePojo>>(
                          ...
                          builder: (context, snap) {
                            ...
                            final reenMode = homeData?.reenSportsMode == true;
  ...
  Widget _activeCampaignsEntry() {
    return GestureDetector(
      onTap: () {
        openScreen(context, const CampaignListScreen());
      },
      child: Container(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(languages.campaignMatkasser, style: aeTitle()),
                  const SizedBox(height: 2),
                  Text(
                    'Støtt en lokal klubb og få premium kjøtt',
                    style: aeCaption(),
                  ),
                ],
              ),
  ...
  Widget _reenSportsMatkasseCard(ActiveSportsClub club) {
    final clubImage = _resolveClubImageUrl(club.image);
    return Container(
      width: double.infinity,
  ...
  String _resolveClubImageUrl(String image) {
    final trimmedImage = image.trim();
    if (trimmedImage.isEmpty) return '';
    ...
    final base = BaseUrl.domain;
    if (trimmedImage.startsWith('/')) {
      return '$base${trimmedImage.substring(1)}';
    }

    if (trimmedImage.startsWith('assets/')) {
      return '$base$trimmedImage';
    }

    if (!trimmedImage.contains('/')) {
      return '${base}assets/images/store-images/475-384/$trimmedImage';
    }
