      ...
      child: ClipRRect(
        ...
        child: isNetworkImage && image.isNotEmpty
            ? CachedNetworkImage(
                ...
                width: width,
                height: height,
                placeholder: (context, url) => placeholder ?? Container(color: colorShimmerBg),
                errorWidget: (context, url, error) => getErrorHolder(),
...
class LoadImageSimple extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bool isSvgAsset =
        !checkImageUrl() && image.toLowerCase().endsWith('.svg');
    if (isSvgAsset) {
      return SvgPicture.asset(
        ...
        placeholderBuilder: (BuildContext ctx) => SizedBox(
          width: width,
          height: height,
          child: Container(color: colorShimmerBg),
        ),
      );
    }
    return Image(
  ...
  getImageProvider() {
    return checkImageUrl() ? CachedNetworkImageProvider(image) : AssetImage(image);
  }
}
