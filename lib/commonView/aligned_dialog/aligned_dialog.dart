...
Future<T?> showAlignedDialog<T>({
  ...
  Offset position = targetBox.localToGlobal(targetAnchor.alongSize(targetBox.size), ancestor: overlay);

  if (isGlobal) {
    position = overlay.localToGlobal(followerAnchor.alongSize(overlay.size));
  }

  return Navigator.of(context, rootNavigator: useRootNavigator).push<T>(AlignedDialogRoute<T>(
...
class AlignedDialogRoute<T> extends RawDialogRoute<T> {
  ...
  AlignedDialogRoute({
    ...
    Color super.barrierColor = Colors.transparent,
    ...
    Offset offset = Offset.zero,
          ...
          transitionBuilder: transitionsBuilder ?? _buildMaterialDialogTransitions,
        );
}
...
class _FollowerDialogRouteLayout extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    // direction.
    return BoxConstraints.loose(constraints.biggest).deflate(EdgeInsets.only(top: topPadding, bottom: bottomPadding));
  }
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    if (avoidOverflow) {
      ...
      if (rst.dx + childSize.width > size.width) rst = Offset(size.width - childSize.width, rst.dy);
      if (rst.dy + childSize.height > size.height) rst = Offset(rst.dx, size.height - childSize.height);
    }
    return rst;
  }
