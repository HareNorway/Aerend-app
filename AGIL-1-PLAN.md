...
> **Pre-existing failures, not from this phase.** `php artisan test --group=requires-mysql` reports 14 failures in `FeedTokenTest` and the feed-store internal API tests, all HTTP 500 from `Unknown column 'users.deleted_at'` / `'providers.deleted_at'`. The local MySQL test database is missing the soft-delete columns the models expect. Verified identical on a clean tree (`git stash`), so it is test-database drift rather than code: 185 passed / 14 failed before this phase, 215 passed / 14 failed after. Fixing the test DB schema belongs with Phase 9 T6, which touches feed-JWT minting anyway.
